...
class DonorController extends Controller
{
    public function donor(){
        $donors=User::where('role', 'donor')->paginate(20);
        return view('backend.pages.donor.donor',compact('donors'));
    }

    // public function donor_create(){
    //     $user=User::all();
    //     return view('backend.pages.donor.create',compact('user'));
    // } 
    ...
    {
        $data=User::where('role', 'donor')->find($id);
        return view('backend.pages.donor.view',compact('data'));
    }
}
